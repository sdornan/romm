...
The expression mirrors 0098's numeric derivations: MariaDB unquotes before the
numeric CAST so a STORED INSERT does not trip strict-mode truncation, and both
dialects gate on a digits-only value so a malformed blob yields NULL rather
than aborting a write.

Revision ID: 0122_hltb_main_story_column
...
revision = "0122_hltb_main_story_column"
