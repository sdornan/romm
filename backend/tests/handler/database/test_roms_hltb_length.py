...
`generated_hltb_main_story` (migration 0122) materializes the seconds buried in
the `hltb_metadata` blob so the length sort walks an index and the range filter
composes into SQL. A rom HowLongToBeat has no time for derives NULL, which is
what keeps it out of a filtered result.
"""
